...
df=pd.read_csv("/path/interface_area.csv")
...
# Anotate significance
for i, (g1, g2) in enumerate(significant_pairs):
    x1, x2 = pos[g1], pos[g2]
    y1 = df_long.loc[df_long['Group']==g1, 'Value'].max()
    y2 = df_long.loc[df_long['Group']==g2, 'Value'].max()
    y = max(y1, y2) + (df_long['Value'].max() - df_long['Value'].min()) * 0.6
    ax.plot([x1, x2], [y, y], lw=1, color='black')
    ax.text((x1 + x2) / 2, y, get_star(p_value), ha='center', va='bottom', fontsize=8, color='black')

plt.title('Comparison of Interface Area')
plt.xlabel('Group')
plt.ylabel('Interface Area (Å²)')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("/path/interface_area_violin.png", dpi=300, bbox_inches='tight')
plt.show()
